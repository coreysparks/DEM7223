...
Unlike the life-table and discrete time methods of estimating survival, which lumped time into discrete periods, K-M uses the information contained in the actual durations.
...
### Variance in the K-M Etimates
Since the K-M survival function is a statistical estimate, it also has uncertainty to it. To measure the uncertainty, or variance in the estimate, the tradiational method is to use the Greenwood formula.
...
You may not be familiar with this test, but in general, if you look at a plot, and if you think there's a difference, say between two lines, there ususally is, and many times the human eye is a more discerning test than anything.
