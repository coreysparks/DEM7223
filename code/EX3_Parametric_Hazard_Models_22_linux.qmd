...
fit.5b<-pchreg(Surv(secbi, b2event)~educ.high+partnerhiedu,
             data=sub2,
             cuts=seq(1, 300, 24))
summary(fit.5b)
...
-2*fit.5b$loglik[2]+length(fit.5b$coefficients) #have to construct this ourselves
```
...
```{r}
eclskk5<-readRDS("/home/corey/ExpanDrive/OneDrive Business/classes/dem7223/dem7223_22/data/eclskk5.rds")
...
fit<-survfit2(Surv(time = age, event = povtran)~mlths, e.long1)
summary(fit)
fit%>%
