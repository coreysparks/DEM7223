...
$$h(t_i) = h_0 \text{ } g(x_i)$$
usually letting $g(x_i) = \text{exp } (x_i ' \beta)$
...
Linear regression, using the Normal distribution  $$y \sim Normal (b_0+b_1*x, \sigma^2_e)$$
logistic regression y=log(p/1-p)=b0+b1*x
In both of these models, we estimate the parameters, b0 and b1 to describe how x affects y


In Parametric hazard models, we estimate parameters that describe the shape of the distribution of duration times
E.g. the normal distribution function is defined by 2 parameters: μ and σ
Which define the characteristic
 bell-shaped curve
...
More parameters allow for more flexibility to the shape of the distribution, and hence more flexibility when it comes to fitting the distribution to data
...
### Connecting distributions to data
Let's consider the exponential distribution as an example of how to fit a distribution to duration data.
The exponential's survival time functions are



Since λ is constant over time, the hazard is a flat line, or constant for all time.  
If a plot of the K-M hazard is not flat over time don't use this model!

### More on the exponential model
### AFT form
Since the exponential distribution is solely determined by the parameter, λ, and λ>0, we need a model to accommodate this.
...
#### PH form
If we treat the hazard rate, λ, as a function of the covariates and the β's, we can write λ as
λi=exp (- xi 'β)
...
If x is either 1 or 0, and the first term in the β's is a constant (the intercept term), we can write our hazard model as:

So β0  is a constant, called the baseline hazard
Changes to this baseline hazard happen through the effect of β1, or the covariate effects, we can consider the relative change in the hazard for someone with x1 =1, verses someone with x1 = 0

This is known as the proportional hazards property

Since the hazard rate in the Exponential model is 
invariant with respect to time, it represents a very simplistic model
 and one that often does not occur in the real world


### Weibull model
Since the Weibull distribution is determined by two parameters, it allows us more flexibility in our modeling
The survival time functions for the Weibull are

Similar to the exponential model, the Weibull model is parameterized in terms of covariates as a log-linear model of time, or the AFT parameterization
log(T) = x'β+σε
Where the β's are regression parameters relating covariate values (the x's) to the duration time, 
ε is an error term corresponding to an extreme value distribution, and σ is scaling term for the error distribution equal to 1/p 

Like the exponential, however, we can also express the Weibull in terms of the conditional hazard rate

This parameterization of the Weibull model gives us the proportional hazards model
In estimation, the term exp(x'β) can be thought of as the scale parameter and p as the shape parameter

### Be careful with interpretations!
For Accelerated failure time model
Y=log (duration), so if exp(β) >1, you have an increase in time (implies a decrease in risk), if exp(β)<1 you have a decrease in time (and an implied increase in risk)
For Proportional Hazards model
Y=hazard(time), so if exp(β) >1, you have an increase in hazard (and a decrease in duration), if exp(β)<1 you have a decrease in hazard (and an increase in duration)
