...
![](C:/Users/ozd504/Documents/Github/DEM7223/images/weibull.png)
...
sub<-dat %>%
  ...
  transmute(CASEID=caseid, 
                 ...
                 psu=v021,
                 strata=v022)%>%
  select(CASEID, int.cmc, fbir.cmc, sbir.cmc, marr.cmc, rural, educ, age, partneredu, partnerage, weight, psu, strata)%>%
...
fit.1<-phreg(Surv(secbi, b2event)~educ.high+partnerhiedu+age+age2,
             data=sub2,
             dist="weibull",
             shape = 1)
summary(fit.1)
...
fit.2<-phreg(Surv(secbi, b2event)~educ.high+partnerhiedu+age+age2,
             data=sub2,
             dist="weibull")
summary(fit.2)
plot(fit.2)
...
fit.3<-phreg(Surv(secbi+.0001, b2event)~educ.high+partnerhiedu+age+age2,
             data=sub2,
             dist="lognormal")

summary(fit.3)
plot(fit.3, ylim=c(0, 1))
...
```{r}
#log-normal distribution for hazard
fit.4<-phreg(Surv(secbi, b2event)~educ.high+partnerhiedu+age+age2,
             data=sub2,
             dist="loglogistic")
summary(fit.4)
plot(fit.4)
...
fit.5<-pchreg(Surv(secbi, b2event)~educ.high+partnerhiedu+age+age2,
             data=sub2,
             cuts=seq(1, 300, 12))
summary(fit.5)
plot(fit.5)
...
```{r}
emp<-coxreg(Surv(secbi, b2event)~educ.high+partnerhiedu+age+age2,
            data=sub2)
...
#Piecewise constant
fitl3<-phreg(Surv(time = time, event = povtran)~mlths+mgths+race_rec,data=e.long, dist = "lognormal")
summary(fitl3)  
