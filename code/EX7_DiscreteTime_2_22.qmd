...
# fit.g<-svyglm(b2event~as.factor(year)-1,
#               design=des,
...
library(car)
library(tidyverse)
dat<-haven::read_dta("/home/corey/ExpanDrive/OneDrive Business/classes/dem7223/dem7223_22//data/nhis_00012.dta")
names(dat)
...
sub <- sub%>%
  select(race_eth, d.age, d.event, college, psu, strata, mortwtsa)%>%
...
newdat$haz_0<-as.numeric(predict(m0, newdata=newdat, type="response"))
newdat$haz_1<-as.numeric(predict(m1, newdata=newdat, type="response"))
newdat$haz_2<-as.numeric(predict(m2, newdata=newdat, type="response"))
newdat$haz_3<-as.numeric(predict(m3, newdata=newdat, type="response"))
newdat$haz_4<-as.numeric(predict(m4, newdata=newdat, type="response"))
...
library(magrittr)
 ...
 out2 <- pivot_longer(newdat, cols = c(-mort_5_yr, -race_eth, -college),  
                     names_to  = c(".value", "mod"), 
                names_sep = "_")
# out<-melt(setDT(newdat),
#           id = c("mort_5_yr", "race_eth", "college"),
#           measure.vars = list(haz=c("h_0", "h_1","h_2","h_3", "h_4")))
head(out2, n=20)
...
out2%>%
  ...
  dplyr::mutate(mod = dplyr::case_when(.$mod =="0" ~ "Constant",
                          .$mod =="1" ~ "General",
                          .$mod =="2" ~ "Linear",
                          .$mod =="3" ~ "Polynomial - 2",
                          .$mod =="4" ~ "Spline"))%>%
  ggplot(aes(x = mort_5_yr*5, y=haz ))+
  ...
  xlab("Age")+ylab("S(t)")+xlim(50, 80)+
  facet_wrap(~mod, scales = "free_y")#+ scale_y_continuous(trans = "log10")
```
