...
![](C:/Users/ozd504/Github/DEM7223/images/surv_ex.png)

![](C:/Users/ozd504/Github/DEM7223/images/H_ex.png)
...
head(e.long1[which.max(e.long1$rrisk),c("childid", "age_enter", "mlths", "mgths","race_rec", "risk")], n=20)

tail(e.long1[which.min(e.long1$rrisk),c("childid", "age_enter", "mlths", "mgths","race_rec", "risk")], n=20)

```


So, the first of these children has a risk score of `1.614` which means their risk was 61% higher that of the baseline child. Likewise, the lowest risk child had a risk score of `0.103` that means their score is almost 90% less than the baseline category.
...
#Cox Model
#interval censored
fitl2<-coxreg(Surv(time = as.numeric(wave), event = povtran)~mlths+mgths+race_rec, data=e.long1)
summary(fitl2)  
...
#Fit the model
fitl2s<-svycoxph(Surv(time = as.numeric(wave), event = povtran)~mlths+mgths+race_rec, design=des2)
summary(fitl2s) 
```
